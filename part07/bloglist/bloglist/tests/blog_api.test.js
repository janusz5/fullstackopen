...
describe("backend api", () => {
  ...
  test("updating a blog", async () => {
    ...
    const putResponse = await api
      .put(`/api/blogs/${unchangedBlog.id}`)
      .send(changedBlogObject);
  ...
  test("incorrect user creation", async () => {
    const onlyUsernameResponse = await api
      .post("/api/users")
      .send({ username: "user" });
    expect(onlyUsernameResponse.status).toEqual(400);
    expect(onlyUsernameResponse.body.error).toMatch(
      /username and password need to be given with at least 3 characters/
    );

    const onlyPasswordResponse = await api
      .post("/api/users")
      .send({ password: "password" });
    expect(onlyPasswordResponse.status).toEqual(400);
    expect(onlyPasswordResponse.body.error).toMatch(
      /username and password need to be given with at least 3 characters/
    );

    const shortUsernameResponse = await api
      .post("/api/users")
      .send({ username: "u", password: "password" });
    expect(shortUsernameResponse.status).toEqual(400);
    expect(shortUsernameResponse.body.error).toMatch(
      /username and password need to be given with at least 3 characters/
    );

    const shortPasswordResponse = await api
      .post("/api/users")
      .send({ username: "user", password: "p" });
    expect(shortPasswordResponse.status).toEqual(400);
    expect(shortPasswordResponse.body.error).toMatch(
      /username and password need to be given with at least 3 characters/
    );
  });

  test("correct user creation", async () => {
    const correctResponse = await api
      .post("/api/users")
      .send({ password: "password", username: "user", name: "User Name" });
